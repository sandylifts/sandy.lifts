...
const CARDS = [
  {
    ...
    linkHoverClass: "group-hover:text-[#A855F7]"
  },
  {
    num: "02",
  ...
  },
  {
    ...
    linkHoverClass: "group-hover:text-[#22D3EE]"
  },
  {
    num: "04",
...
const TESTIMONIALS = [
  { initial: "P", text: "Priya lost 4.5 kg in 6 weeks — no confusion, just structure", avatarBg: "linear-gradient(135deg, #A855F7, #7C3AED)" },
  { initial: "R", text: "Rahul gained 3kg muscle in 8 weeks — plan-driven progress", avatarBg: "linear-gradient(135deg, #22D3EE, #06B6D4)" },
  { initial: "A", text: "Amit lost 8kg in 12 weeks — results he could actually maintain", avatarBg: "linear-gradient(135deg, #3D1A6E, #111111)" }
];
...
export function WhySection() {
  ...
  return (
    <section className="relative w-full pt-8 pb-24 bg-[#000000] overflow-hidden font-['DM_Sans',sans-serif]">
      {/* Global CSS for Animations & Gradients */}
      <style>{`
        @keyframes marquee {
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 relative z-10 flex flex-col items-center">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <span className="text-[9px] font-bold tracking-[0.25em] text-[#C084FC] uppercase">
            THE SANDY.LIFTS SYSTEM
          </span>
          <div className="w-12 h-[1px] bg-gradient-to-l from-transparent to-[#7C3AED]" />
        </div>

        {/* 3. Main Heading */}
        <h2 className="font-syne font-[800] text-center leading-[1.1] mb-6 flex flex-col items-center" style={{ fontSize: 'clamp(28px, 4.5vw, 44px)' }}>
          <span className="text-[#FFFFFF]">What Makes This</span>
          <span className="text-gradient-primary">System Different?</span>
        </h2>

        {/* 4. Subtext */}
        <p className="text-[#A1A1AA] text-[13px] text-center max-w-[420px] mb-10 leading-relaxed">
          Results don’t come from motivation. They come from systems.
        </p>

        {/* 5. Trust Strip - Infinite Marquee Slider */}
        <div className="w-full relative flex overflow-hidden mb-20 max-w-[1400px]" style={{ maskImage: "linear-gradient(to right, transparent, black 10%, black 90%, transparent)", WebkitMaskImage: "linear-gradient(to right, transparent, black 10%, black 90%, transparent)" }}>
          <div className="flex w-max animate-marquee-slow hover:[animation-play-state:paused]">
            {[...Array(4)].map((_, arrayIndex) => (
              <div key={arrayIndex} className="flex items-center gap-4 px-2">
                {TESTIMONIALS.map((test, i) => (
                  <div 
                    key={`${arrayIndex}-${i}`} 
                    className="flex items-center gap-3 bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.06)] rounded-full px-5 py-2.5 hover:bg-[rgba(255,255,255,0.05)] hover:border-[rgba(255,255,255,0.15)] transition-all cursor-default group"
                  >
                    <div 
                      className="w-7 h-7 shrink-0 rounded-full flex items-center justify-center text-[11px] font-bold text-white shadow-inner transition-transform group-hover:scale-110"
                      style={{ background: test.avatarBg }}
                    >
                      {test.initial}
                    </div>
                    <span className="text-[12px] font-medium text-[#AAB3C5] whitespace-nowrap tracking-wide">
                      {test.text.split('—').map((part, pIndex) => (
                        <span key={pIndex}>
                          {pIndex === 0 ? (
                            <span className="text-[#F5F7FA] font-semibold">{part}</span>
                          ) : (
        ...
        {/* 6. Features Grid & 7. System Flow Line */}
        <div className="relative w-full max-w-[1200px] mb-0">
          {/* System Flow Line (Desktop Only) */}
          ...
          {/* System Flow Line (Mobile/Tablet) */}
            ...
            {CARDS.map((card, i) => (
              ...
              >
                ...
                >
                  ...
                  <div 
                    className="absolute top-0 left-0 w-full h-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300 rounded-t-2xl"
                    style={{ background: `linear-gradient(90deg, transparent, ${card.hoverBorder}, transparent)`, boxShadow: `0 0 15px ${card.hoverGlow}` }}
                  />

                  {/* Core System Badge for Hero */}
                  {card.isHero && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 bg-[rgba(168,85,247,0.15)] border border-[rgba(168,85,247,0.3)] rounded-full px-3 py-1 backdrop-blur-md">
                      <div className="w-1.5 h-1.5 rounded-full bg-[#C084FC] animate-blink shadow-[0_0_8px_#C084FC]" />
                      <span className="text-[8px] font-bold tracking-widest text-[#C084FC] uppercase">CORE SYSTEM</span>
                    </div>
                  )}

                  {/* Number */}
                  <div className="absolute top-4 right-5 text-[#52525B] font-jetbrains text-sm font-medium transition-colors duration-300 group-hover:text-[#C084FC]">
                    {card.num}
                  </div>
                  ...
                  {/* Content */}
                  <h3 className="text-[17px] font-syne font-bold text-[#FFFFFF] mb-3 leading-tight">
                    {card.title.split(card.accentWord).map((part, index, array) => (
                      <span key={index}>
                        {part}
                        {index < array.length - 1 && (
                          <span style={{ color: card.accentText }}>{card.accentWord}</span>
                        )}
                      </span>
                    ))}
                  </h3>
                  <p className="text-[13px] text-[#A1A1AA] leading-relaxed mb-6 flex-grow">
                    {card.desc}
                  </p>

                  {/* Link */}
                  <Link href="#" className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#52525B] transition-colors mt-auto w-max group/link relative z-10">
                    <span className={`${card.linkHoverClass} transition-colors`}>→ learn more</span>
                  </Link>
                </div>
              </motion.div>
            ))}
        ...
        {/* 8. Quote + Stats bar */}
        <div className="w-full max-w-[1200px] mx-auto bg-[#111111] rounded-b-2xl border border-[rgba(168,85,247,0.12)] border-t-0 flex flex-col md:flex-row relative z-10 overflow-hidden">
          ...
          <div className="flex-1 p-8 md:p-12 relative flex flex-col justify-center border-b md:border-b-0 md:border-r border-[rgba(168,85,247,0.12)]">
            ...
            <p className="text-xl md:text-2xl font-syne font-bold text-[#FFFFFF] relative z-10 pl-6 leading-tight mb-4">
            ...
            <p className="pl-6 text-[13px] text-[#52525B] font-medium tracking-wide">
              — Sandy, Founder & Head Coach
            </p>
          </div>
          ...
          <div className="flex-[1.2] p-8 md:p-12 grid grid-cols-2 sm:grid-cols-4 gap-6 items-start">
            {[
              ...
              <div key={i} className="flex flex-col items-center sm:items-start text-center sm:text-left group cursor-default">
                <div className={`text-3xl md:text-4xl font-syne font-[900] mb-1 transition-transform duration-300 group-hover:-translate-y-[2px] ${stat.color}`}>
                ...
                <div className="text-[12px] font-bold text-[#A1A1AA] mb-1 uppercase tracking-wider">
                  {stat.label}
                </div>
              ...
              {MARQUEE_WORDS.map((word, i) => {
                ...
                return (
                  <div key={i} className="flex items-center">
                    <span className={`text-[13px] font-syne font-bold tracking-[0.2em] uppercase px-8 ${colorClass}`}>
                      {word}
                    </span>
