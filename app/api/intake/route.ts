  ...
  try {
    ...
    if (supabaseUrl && supabaseKey) {
      ...
      await sb.from('intake_submissions').insert({
        ...
        lifestyle:  JSON.stringify({ sleep: formData.sleepHours, stress: formData.stressLevel, water: formData.waterIntake, job: formData.jobType }),
        diet:       JSON.stringify({ food: formData.foodType, meals: formData.mealsPerDay, dislikes: formData.foodDislikes, junk: formData.junkHabit, lateNight: formData.lateNightEating }),
        fitness:    JSON.stringify({ experience: formData.experience, activity: formData.currentActivity, time: formData.timeAvailable, place: formData.workoutPlace }),
        goals:      JSON.stringify({ main: formData.mainGoal, focus: formData.focusArea, target: formData.targetInMonth, why: formData.whyTransform }),
